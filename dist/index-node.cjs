...
__export(index_node_exports, {
  ...
  LCMStableDiffusionPipeline: () => LCMStableDiffusionPipeline,
  ProgressStatus: () => ProgressStatus,
  dispatchProgress: () => dispatchProgress,
  getModelFile: () => getModelFile2,
  getModelJSON: () => getModelJSON,
  getModelTextFile: () => getModelTextFile,
  loadModel: () => loadModel,
  sessionRun: () => sessionRun,
  setCacheImpl: () => setCacheImpl,
  setModelCacheDir: () => setModelCacheDir
});
...
function getCacheKey(modelRepoOrPath, fileName, revision) {
  ...
  return import_path.default.resolve(filePath);
}
async function writeResponseToFile(response, displayName, outputPath) {
  ...
  const writeStream = import_fs.default.createWriteStream(outputPath + ".tmp");
  ...
  await import_fs.default.promises.rename(outputPath + ".tmp", outputPath);
}
async function getModelFile(modelRepoOrPath, fileName, fatal = true, options = {}) {
  ...
  if (await fileExists(cachePath)) {
    if (options.returnText) {
      return import_fs.default.promises.readFile(cachePath, { encoding: "utf-8" });
    }
    return cachePath;
  }
  try {
    const response = await (0, import_hub.downloadFile)({ repo: modelRepoOrPath, path: fileName, revision });
    const targetPath = import_path.default.dirname(cachePath);
    if (!await fileExists(targetPath)) {
      await import_fs.default.promises.mkdir(targetPath, { recursive: true });
    }
...
function getModelTextFile(modelPath, fileName, fatal, options) {
  return getModelFile2(modelPath, fileName, fatal, { ...options, returnText: true });
}
...
// src/index-node.ts
var import_backend = require("onnxruntime-node/dist/backend.js");
var ORT2 = __toESM(require("@aislamov/onnxruntime-web64"), 1);

// src/util/Tensor.ts
var import_transformers = require("@xenova/transformers");
var import_seedrandom = __toESM(require("seedrandom"), 1);
import_transformers.Tensor.prototype.reverse = function() {
  return new import_transformers.Tensor(this.type, this.data.reverse(), this.dims.slice());
};
import_transformers.Tensor.prototype.sub = function(value) {
  return this.clone().sub_(value);
};
import_transformers.Tensor.prototype.sub_ = function(value) {
  if (typeof value === "number") {
    for (let i = 0; i < this.data.length; ++i) {
      this.data[i] -= value;
    }
  } else if (value instanceof import_transformers.Tensor) {
...
import_transformers.Tensor.prototype.add = function(value) {
  return this.clone().add_(value);
};
import_transformers.Tensor.prototype.add_ = function(value) {
  if (typeof value === "number") {
    for (let i = 0; i < this.data.length; ++i) {
      this.data[i] += value;
    }
...
import_transformers.Tensor.prototype.cumprod = function(dim) {
  return this.clone().cumprod_(dim);
};
import_transformers.Tensor.prototype.cumprod_ = function(dim) {
...
import_transformers.Tensor.prototype.mul = function(value) {
  return this.clone().mul_(value);
};
import_transformers.Tensor.prototype.mul_ = function(value) {
  if (typeof value === "number") {
    for (let i = 0; i < this.data.length; ++i) {
      this.data[i] *= value;
    }
...
import_transformers.Tensor.prototype.div = function(value) {
  return this.clone().div_(value);
};
import_transformers.Tensor.prototype.div_ = function(value) {
  if (typeof value === "number") {
    for (let i = 0; i < this.data.length; ++i) {
      this.data[i] /= value;
    }
...
import_transformers.Tensor.prototype.pow = function(value) {
  return this.clone().pow_(value);
};
import_transformers.Tensor.prototype.pow_ = function(value) {
  if (typeof value === "number") {
    for (let i = 0; i < this.data.length; ++i) {
...
import_transformers.Tensor.prototype.round = function() {
  return this.clone().round_();
};
import_transformers.Tensor.prototype.round_ = function() {
  for (let i = 0; i < this.data.length; ++i) {
    this.data[i] = Math.round(this.data[i]);
  }
  return this;
};
import_transformers.Tensor.prototype.tile = function(reps) {
  return this.clone().tile_(reps);
};
import_transformers.Tensor.prototype.tile_ = function(reps) {
  ...
  return new import_transformers.Tensor(this.type, newData, newDims);
};
import_transformers.Tensor.prototype.clipByValue = function(min, max) {
  return this.clone().clipByValue_(min, max);
};
import_transformers.Tensor.prototype.clipByValue_ = function(min, max) {
  if (max < min) {
...
import_transformers.Tensor.prototype.exp = function() {
  return this.clone().exp_();
};
import_transformers.Tensor.prototype.exp_ = function() {
  for (let i = 0; i < this.data.length; ++i) {
    this.data[i] = Math.exp(this.data[i]);
  }
  return this;
};
import_transformers.Tensor.prototype.sin = function() {
  return this.clone().sin_();
};
import_transformers.Tensor.prototype.sin_ = function() {
  for (let i = 0; i < this.data.length; ++i) {
    this.data[i] = Math.sin(this.data[i]);
  }
  return this;
};
import_transformers.Tensor.prototype.cos = function() {
  return this.clone().cos_();
};
import_transformers.Tensor.prototype.cos_ = function() {
  for (let i = 0; i < this.data.length; ++i) {
    this.data[i] = Math.cos(this.data[i]);
  }
  return this;
};
import_transformers.Tensor.prototype.location = "cpu";
function range(start, end, step = 1, type = "float32") {
  const data = [];
  for (let i = start; i < end; i += step) {
    data.push(i);
  }
  return new import_transformers.Tensor(type, data, [data.length]);
}
function linspace(start, end, num, type = "float32") {
  ...
  return new import_transformers.Tensor(type, arr, [num]);
}
...
function randomNormalTensor(shape, mean = 0, std = 1, type = "float32", seed = "") {
  const data = [];
  const rng = seed !== "" ? (0, import_seedrandom.default)(seed) : (0, import_seedrandom.default)();
  for (let i = 0; i < shape.reduce((a, b) => a * b); i++) {
    data.push(randomNormal(rng) * std + mean);
  }
  return new import_transformers.Tensor(type, data, shape);
}
function cat(tensors, axis = 0) {
  ...
  return new import_transformers.Tensor(tensorType, data, tensorShape);
}
function replaceTensors(modelRunResult) {
  const result = {};
  for (const prop in modelRunResult) {
    if (modelRunResult[prop].dims) {
      result[prop] = new import_transformers.Tensor(
        // @ts-ignore
...
function betasForAlphaBar(numDiffusionTimesteps, maxBeta = 0.999, alphaTransformType = "cosine") {
  ...
  return new import_transformers2.Tensor(betas);
}
...
var import_transformers3 = require("@xenova/transformers");
var LCMScheduler = class extends SchedulerBase {
  ...
  setTimesteps(numInferenceSteps) {
    ...
    this.timeIndex = 0;
    this.timesteps = new import_transformers3.Tensor(
      new Int32Array(
        timesteps
      )
    );
...
var import_transformers4 = require("@xenova/transformers");
var CLIPTokenizer = class _CLIPTokenizer extends import_transformers4.PreTrainedTokenizer {
  bos_token_id;
  eos_token_id;
  ...
  _call(text, {
    ...
    if (return_tensor) {
      ...
      if (return_tensor_dtype === "int32") {
        tokens = new import_transformers4.Tensor(
          return_tensor_dtype,
          Int32Array.from(tokens.flat()),
          dims
        );
        attention_mask = new import_transformers4.Tensor(
          return_tensor_dtype,
...
var import_transformers6 = require("@xenova/transformers");

// src/backends/index.ts
var ORT = __toESM(require("onnxruntime-react-native"), 1);
...
var import_transformers5 = require("@xenova/transformers");
var PipelineBase = class {
  ...
  async encodePrompt(prompt) {
    ...
    const encoded = await this.textEncoder.run({ input_ids: new import_transformers5.Tensor("int32", Int32Array.from(inputIds.flat()), [1, inputIds.length]) });
...
var LCMStableDiffusionPipeline = class _LCMStableDiffusionPipeline extends PipelineBase {
  ...
  getWEmbedding(batchSize, guidanceScale, embeddingDim = 512) {
    let w = new import_transformers6.Tensor("float32", new Float32Array([guidanceScale]), [1]);
    w = w.mul(1e3);
  ...
  async run(input) {
    ...
    for (const step of timesteps) {
      const timestep = input.sdV1 ? new import_transformers6.Tensor(BigInt64Array.from([BigInt(step)])) : new import_transformers6.Tensor(new Float32Array([step]));
      await dispatchProgress(input.progressCallback, {
  ...
  async encodeImage(inputImage, width, height) {
    const encoded = await this.vaeEncoder.run(
      { sample: new import_transformers6.Tensor("float32", inputImage, [1, 3, width, height]) }
    );
...
// src/hub/browser.ts
var import_hub6 = require("@huggingface/hub");
...
// src/index-node.ts
var ONNX2 = ORT2.default ?? ORT2;
ONNX2.registerBackend("cpu", import_backend.onnxruntimeBackend, 1002);
setCacheImpl(node_default);
