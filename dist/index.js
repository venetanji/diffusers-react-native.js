(() => {
  ...
  // src/util/Tensor.ts
  var import_transformers = __require("@xenova/transformers");
  var import_seedrandom = __toESM(__require("seedrandom"), 1);
  ...
  var DbCache = class {
    ...
    db;
    init = async () => {
      const openRequest = await (0, import_idb.openDB)(this.dbName, this.dbVersion, {
        upgrade(db) {
  ...
  async function getModelFile2(modelRepoOrPath, fileName, fatal = true, options = {}) {
    ...
    try {
      if (!response) {
        response = await (0, import_hub2.downloadFile)({ repo: modelRepoOrPath, path: fileName, revision });
      }
      if (!response || !response.body || response.status !== 200 || response.headers.get("content-type")?.startsWith("text/html")) {
  ...
  var import_transformers2 = __require("@xenova/transformers");
  ...
  var import_transformers3 = __require("@xenova/transformers");
  ...
  var import_transformers4 = __require("@xenova/transformers");
  ...
  var import_transformers6 = __require("@xenova/transformers");

  // src/pipelines/PipelineBase.ts
  var import_transformers5 = __require("@xenova/transformers");
